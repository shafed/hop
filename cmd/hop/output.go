...
//
// Узлы и группы берутся готовыми из store.GroupNodesView: список показываемых
// полей — явное перечисление того, что показывать МОЖНО (Р12, §6.14), и живёт
// оно в сторе, где его сторожит S33. Повторить его здесь значило бы завести
// второй экземпляр правды про секреты.
//
// Тот же тип едет и через сокет §3.3 (agent.Client.Nodes): у команды один
// адресат из двух — связка при живом агенте, стор без него, — и схема `--json`
// от выбора адресата зависеть не имеет права. Ровно так же `hop status`
// печатает готовый agent.ClientStatus.
type nodesOut struct {
	Groups []store.GroupNodesView `json:"groups"`
}
...
// nodesView собирает вывод `hop nodes` из стора — путь без живого агента.
//
// Сборка целиком в сторе (FullView), а не здесь по store.Group: форматирование
// времени и выбор показываемых полей — работа стора, и второй её экземпляр в
// cmd/hop разошёлся бы с первым молча. Связка зовёт ту же FullView, только
// подаёт ей живость свежее диска.
//
// nil вместо живости означает «свежее нечему быть»: агента нет, и последнее
// известное состояние узлов — то, что лежит на диске (§2).
func nodesView(st *store.Store) nodesOut {
	return nodesOut{Groups: st.FullView(nil)}
}
...
// существовало. Второй довод — свежесть: стор хранит СРЕЗ живости (§2),
// доходящий до диска не чаще раза в тридцать секунд, и команда, которая
// собирает ответ из стора, показывает получасовой давности состояние ровно
// тогда, когда актуальное есть у того, кого она не спросила.
//
// Довод здесь переписан: раньше вторым стояло «стор держит эксклюзивный flock
// всё время жизни агента, поэтому команда отказывает при поднятом туннеле».
// Замер это опроверг — см. implementation-notes.md, «Замер: держит ли агент
// flock всю жизнь».
//
