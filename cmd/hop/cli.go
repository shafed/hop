...
var commands = []*command{
	...
	{
		verb:  "nodes",
		help:  "узлы и группы: у связки, пока она жива, иначе из стора",
		reads: true,
		setup: clientFlags,
		run:   (*cli).runNodes,
	},
	...
	},
	{
		...
		// За сокет §3.3 эта команда НЕ уходит, в отличие от `nodes`, и это
		// решение, а не недоделка. Три довода, из которых решает третий.
		//
		//  1. Она существует ради случая, когда связки нет или ей не верят:
		//     §5.9 называет её «нужной ровно там, где отказ агента не
		//     отвечает на вопрос, чей он». Диагностика последней надежды,
		//     идущая через диагностируемого, отвечает на другой вопрос.
		//  2. Она — единственное место, где код 3 наблюдаем БЕЗ агента, и на
		//     этом стоит W56. Уведи её в сокет — и проверка кодов возврата
		//     станет требовать поднятой связки, то есть перестанет проверять
		//     границу между кодом 2 и кодом 3.
		//  3. При живом агенте она не нужна: §6.5 уже опрашивает те же узлы
		//     по расписанию, и вторая проба поверх неё — это ровно тот
		//     «полный обход подписки», против которого §6.5 и написан (4000
		//     соединений в час против 870). Живую картину показывают `status`
		//     и `nodes`, и обе теперь берут её у связки.
		//
		// Замок при этом ей не мешает: чтение стора идёт без замка (см. замер
		// в implementation-notes.md, «Замер: держит ли агент flock всю
		// жизнь»).
		reads: true,
		setup: func(fs *flag.FlagSet, o *options) {
...
// runNodes — §1/С2: состав стора с живостью.
//
// Спрашивается связка, а стор остаётся запасным адресатом — тот же порядок,
// что у `status`, и по той же причине: живость знает связка. Стор держит СРЕЗ
// живости (§2), доходящий до диска не чаще раза в тридцать секунд, поэтому при
// живом агенте прямое чтение стора показывает устаревшее состояние там, где
// актуальное есть у того, кого не спросили.
//
// Но, в отличие от `status`, запасной адресат здесь не сервис, а сам стор, и
// это не обходной путь, а буквальное исключение §3.3: «`hop sub add` из §1/С2
// существует раньше `hop up`, то есть раньше живого агента». Команда, которая
// показывает добавленное, обязана работать там же, где работает добавляющая, —
// то есть до первого запуска агента.
//
// Отказ связки на этом пути поэтому не код 2: агента может не быть законно.
func (c *cli) runNodes(o *options, _ []string) error {
	...
	if err == nil {
		...
		return emit(c.stdout, nodesOut{Groups: groups}, o.json)
	}
