...
func (a *Agent) Snapshot() Snapshot {
	...
	// Pinned до сокета §3.3 не заполнялся ни разу: поле снимка было пустым
	// всегда, а §1/С5 требует показывать фиксацию отдельной строкой.
	//
	// Считается, а не хранится своим полем: фиксация в живости и есть
	// «автовыбор выключен, активный узел не заменяется» — `health.Pin` ставит
	// `auto=false` и `active=id` одним присваиванием, а `health.Auto(false)`
	// даёт то же наблюдаемое поведение. Отдельное поле было бы вторым
	// источником правды о том, что уже выражено парой (Auto, Active), и
	// разошлось бы с ней на первом же `hop auto off`.
	if !s.Auto {
		s.Pinned = s.Active
	}
