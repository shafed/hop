...
//
// Замер на настоящем сторе с 200 узлами (`TestW65MeasureNodesAgainstTheFrameLimit`
// в cmd/hop, разбор в implementation-notes.md): ответ целиком весит **64941
// байт** — на 595 байт, то есть меньше чем на процент, меньше предела. Кадр
// переполняется на 202 узлах той же формы, а если удлинить имя каждого узла на
// десять символов — уже на 200. То есть одним кадром обещание §6.5 держится
// ровно до опечатки, и держится случайно.
//
