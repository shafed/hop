...
type Config struct {
	...
	// Acked зовётся после того, как событие обработано — сброшен кэш или
	// принято решение его не сбрасывать. Нужен связке: Р33 требует, чтобы кэш
	// был выкинут **раньше**, чем событие ушло наружу, иначе приложение,
	// разбуженное событием, успеет получить адрес, добытый через мёртвый узел.
	//
	// Зовётся всегда, даже когда сброса не было: иначе выключенная политика
	// dns_cache_flush_on_switch подвешивала бы связку вместо того, чтобы
	// покраснить проверку.
	Acked func()
