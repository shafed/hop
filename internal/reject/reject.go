...
// RST строит только TCP RST и не спрашивает Excluded — или nil, если ответа
// не положено (пакет не TCP, мусор, сам RST).
//
// Зачем отдельно от Reply. Excluded отвечает на вопрос «на что респондер §5.6
// не имеет права отвечать»: там пакет не отвергнут, он просто не наше дело, и
// локальная сеть в этом списке первой строкой. Но вердикт bypass в netstack
// решает про ту же локальную сеть обратное: поток отвергнут, потому что путь
// bypass — UDP-NAT и TCP не несёт (§6.10, §6.8), и отказ здесь не имеет к
// fail-close отношения. Если бы такой поток шёл через Reply, Excluded вернул бы
// nil, и §5.6 нарушился бы там, где его же список призван был помочь.
//
// Никакой политики внутри: кому отвечать — решает вызывающий, здесь только
// заголовки. Тот же tcpRST, что у Reply, — второй копии RST в продукте нет.
func RST(pkt []byte) []byte {
	ip, ok := parseIPv4(pkt)
	if !ok || ip.Protocol() != uint8(header.TCPProtocolNumber) {
		return nil
	}
	return tcpRST(ip, ip.Payload())
}
