...
// Блокировать диалер тоже нельзя: он вызывается из обработчика форвардера, то
// есть с цикла чтения устройства, и ожидание в нём остановило бы весь туннель.
// Молчание здесь §5.6 не нарушает: тот запрещает молчание там, где решение
// принято, а в стартовом окне оно ещё не принято.
//
// UDP отдельной ветки не требует: там дозвон, не удавшись, роняет датаграмму,
// и клиент повторяет её сам — рукопожатия, которое надо придержать, нет.
...
// ResetFlows рвёт живые потоки и записи NAT и возвращает их число.
//
// Зовёт его супервизор и ровно при переключении по причине dead (§5.5):
// соединения через мёртвый узел уже мертвы, и разрыв даёт приложению
// переподключиться за секунды вместо минут TCP-таймаута. При faster не зовётся
// вовсе — обрыв загрузки ради выигранных 30 мс раздражает сильнее, чем сами
// 30 мс.
//
// Поток TCP закрывается, а не сбрасывается RST: gonet отдаёт net.Conn, у
// которого есть только Close. Приложение видит конец потока немедленно —
// именно этого §5.5 и хочет, в отличие от молчания, которое запрещает §5.6.
//
// Вердикты забываются вместе с потоками: пакет, пришедший следом за разрывом,
// принадлежит потоку, которого больше нет, и вердикт ему нужен новый.
func (s *Stack) ResetFlows() int {
	n := s.tcp.reset() + s.nat.reset()
	s.flows.reset()
	return n
}
