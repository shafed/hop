...
//
// Работает при инварианте стенда: к моменту вызова цикл уже стоит на часах,
// то есть его ожидающий зарегистрирован. Фейковые часы отдают тик только тому,
// кто ждёт в момент Advance (clock/fake.go: Advance перебирает уже собранный
// список), поэтому прокрутка, попавшая в цикл между «проснулся» и «встал
// ждать снова», не достаётся никому. Инвариант держат startAndSettle,
// advanceRound и pin — каждый возвращается только после того, как цикл
// зарегистрировал следующий таймер (Manager.loop делает это ДО round++).
...
// pin фиксирует узел и дожидается витка цикла, который поднял сам Pin.
//
// Pin будит цикл (Manager.poke): сроки проб изменились — зафиксированный узел
// стал активным и обязан опрашиваться как активный, — и цикл пересчитывает
// ближайший срок. Виток холостой: пробовать в этот момент ещё нечего. Но
// раунд он всё равно засчитывает, а таймер регистрирует заново, и без
// ожидания здесь тест уходит крутить часы, пока цикл в этом витке. Тогда
// Advance не достаётся никому, холостой виток закрывает собой ожидание
// advanceRound — и раунд с пробами, которого тест ждал, просто не случается.
// Так краснел A22: узел успевал получить один отказ вместо двух и оставался
// alive. Ждать нужно ровно один раунд: жетон в Manager.wake один, и виток на
// него тоже один.
func (f *fixture) pin(id string) {
	f.t.Helper()
	r := f.mgr.Snapshot().Round
	if err := f.mgr.Pin(id); err != nil {
		f.t.Fatalf("Pin(%s): %v", id, err)
	}
	f.waitRound(r + 1)
}
