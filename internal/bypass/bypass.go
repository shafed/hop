...
// диалером Xray; ключа с dst нет, bypass всегда full-cone; и уборку простоя
// двигает не только следующий пакет, но и фоновый тикер (bypass_idle_sweep).
type NAT struct {
	cfg  Config
	clk  clock.Clock
	idle time.Duration
	...
	// done закрывается один раз в Close и останавливает фоновую уборку
	// простоя. Горутин чтения он не касается: их будит закрытие сокета.
	done      chan struct{}
	closeOnce sync.Once
	wg        sync.WaitGroup
}
...
// startIdleSweeper заводит фоновую уборку простоя (политика bypass_idle_sweep).
// Выключенная политика не оставляет ни горутины, ни тикера: механизма нет
// целиком, а не «есть, но ничего не делает», — иначе выключение флага меняло бы
// только исход, оставляя цену.
func (n *NAT) startIdleSweeper() {
	if !policy.BypassIdleSweep.On() {
		return
	}
	// Тикер заводится здесь, а не в самой горутине, и это не стилистика.
	// Часы могут быть инъектируемыми: заведённый внутри горутины тикер
	// отсчитывал бы первый срок не от New, а от того момента, когда
	// планировщик до горутины дошёл, — и прокрутка часов, обогнавшая запуск,
	// не доставила бы ни одного тика вовсе. Замер: с тикером внутри горутины
	// охраняющая проверка краснела на `go test -count=20 -race`.
	period := n.idle / 2
	if period <= 0 {
		// Idle меньше двух наносекунд — величина не из продукта, но тикер с
		// неположительным периодом паникует, и молчаливо делить на ноль
		// здесь не за что.
		period = n.idle
	}
	t := n.clk.NewTicker(period)
	n.wg.Add(1)
	go n.sweepLoop(t)
}

// sweepLoop прокручивает уборку по часам, а не по чужому событию. До этой
// политики уборку двигал только Send: клиент, который замолчал навсегда,
// оставлял сокет и его горутину чтения жить до Close всего NAT.
//
...
// простоявшего сокета сверх Idle+Idle/2 без всякой пользы.
func (n *NAT) sweepLoop(t clock.Ticker) {
	defer n.wg.Done()
	defer t.Stop()
...
// sweepNow — принудительная прокрутка, как natTable.Sweep()
// (internal/netstack/udp.go): собственное ограничение sweepLocked снимается,
// потому что тикер уже ограничил частоту снаружи, а совпасть с недавним Send
// тик может в любой момент.
func (n *NAT) sweepNow() {
	n.mu.Lock()
	defer n.mu.Unlock()
	if n.closed {
...
// Close останавливает фоновую уборку, закрывает все сокеты и дожидается всех
// заведённых NAT горутин — уборщика и чтения. После Close Send отказывает.
func (n *NAT) Close() {
	n.closeOnce.Do(func() {
		close(n.done)
		n.mu.Lock()
...
// Send и фоновый тикер, но не наблюдатель.
//
...
//
// Цена решения была названа вслух: после того как трафик прекратился, сокеты
// bypass доживали до Close. Она закрыта фоновым тикером (sweepLoop, политика
// bypass_idle_sweep, W51) — то есть поведением со своим флагом и своей
// краснеющей проверкой, а не побочным эффектом геттера. Stats при этом
// осталась чистым чтением: прокручивает уборку тикер, а не наблюдатель.
// natTable (internal/netstack/udp.go) той же участи пока не лишена — у неё
// прокрутка по-прежнему едет на mapping() и на внешний Sweep().
