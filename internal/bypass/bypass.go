...
// natTable.Stats(): фонового тикера у NAT нет, уборка едет либо на Send, либо
// здесь. В продукте это на деле не спасает: ничто снаружи пакета тестов
// сегодня Stats не зовёт (Stack.Stats её не подмешивает — задача на будущее,
// см. HANDOFF.json), так что уборка простаивающих сокетов в продукте идёт
// только на Send и останавливается вместе с трафиком. Это не регрессия: та
// же участь у natTable, чья уборка так же держится на потоке пакетов.
