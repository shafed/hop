...
type Linux struct {
	...
	// marks — след, который этот слой мог оставить в снапшоте сети. Копится
	// по всем Up за жизнь процесса, а не берётся из params: агент вправе
	// перезапуститься с другим именем интерфейса, и течь прошлого воплощения
	// назовёт прошлое имя.
	marksMu sync.Mutex
	marks   []string
}
...
func upSteps(p tunnel.Params, log *slog.Logger) []step {
	...
	if policy.IPv6Block.On() {
		...
		// §5.6: правило закрывает только НОВЫЕ соединения. Установленное до
		// него молчит до собственного таймаута TCP — замер в
		// implementation-notes.md. Зачистка идёт сразу за правилом: раньше
		// она бессмысленна (разрушенное переустановится), позже — окно
		// молчания длиннее без нужды.
		//
		// Отката нет: разрушенный сокет обратно не собрать. Снапшот §8.4
		// это не задевает — он состоит из ссылок, адресов, маршрутов и
		// правил, сокетов в нём нет, — и T22 с T29 остаются про то же, про
		// что были.
		//
		// Условие двойное. Флаг у зачистки свой, потому что она своё
		// утверждение; но без правила она рвала бы работающие соединения
		// IPv6, а не молчащие, — то есть вредила бы. Односторонняя
		// зависимость выражена здесь, а не в документе.
		if policy.IPv6KillEstablished.On() {
			steps = append(steps, step{
				name: stepKillIPv6,
				do:   func() error { sweepSilencedIPv6(log); return nil },
			})
		}
	}
...
func (l *Linux) Up(p tunnel.Params) (tunnel.Device, error) {
	...
	// След записывается ДО первого шага: сорвавшийся на середине Up
	// откатывается сам (ниже), но если и откат не удался — оставленное
	// обязано опознаваться как наше.
	l.addMarks(marks(p))
...
// marks — по каким строкам след hop опознаётся в снапшоте сети (§8.4).
//
// Чистая функция от параметров туннеля, ровно как upSteps и по той же
// причине: «что мы считаем своим» проверяется без прав, иначе убедиться в
// полноте списка можно только привилегированным прогоном.
//
// Список выведен из ЗАМЕРА, а не из чтения upSteps. Замер (в netns
// раскладываются все шаги Up и снимается разница снапшотов, полностью — в
// implementation-notes.md, «hopd и чужие сети») дал 14 строк, и три из них
// upSteps не добавлял вовсе: ядро само завело connected-, local- и
// broadcast-маршруты вслед за адресом. Все три несут имя интерфейса — потому
// имя интерфейса здесь и стоит первым. Список «что мы делали» (журнал) такой
// строки не содержит и содержать не может.
func marks(p tunnel.Params) []string {
	m := []string{
		// Имя интерфейса. Покрывает ссылку, адрес и все маршруты через него,
		// включая заведённые ядром.
		p.Name,
		// Таблица туннеля — в маршруте и в правиле она пишется по-разному.
		"table " + strconv.Itoa(p.Table),
		"lookup " + strconv.Itoa(p.Table),
	}
	// Адрес туннеля с маской и без. Замер показал, что все строки с ним несут
	// и имя интерфейса, то есть здесь он избыточен; оставлен намеренно — на
	// случай платформы или версии iproute2, где это не так. Ошибка в сторону
	// «наше» безопасна, см. netstate.Classify.
	if p.Addr != "" {
		m = append(m, p.Addr)
		if ip, _, ok := strings.Cut(p.Addr, "/"); ok {
			m = append(m, ip)
		}
	}
	// Приоритеты правил: правило-исключение §5.6 не несёт ни имени
	// интерфейса, ни номера таблицы — `31000: from all to 10.0.0.0/8 lookup
	// main», — и опознаётся только по приоритету. legacyPrioLoopGuard здесь
	// тоже: Reclaim обязана подбирать его после обновления со старой сборки,
	// а до Reclaim следующего запуска он остаётся нашим.
	for _, prio := range []int{prioExclusions, legacyPrioLoopGuard, prioTunnel} {
		m = append(m, strconv.Itoa(prio)+":")
	}
	return m
}

func (l *Linux) addMarks(m []string) {
	l.marksMu.Lock()
	defer l.marksMu.Unlock()
	l.marks = append(l.marks, m...)
}

// Footprint — след, который этот слой мог оставить в снапшоте сети.
//
// Пустой, пока ни один туннель не поднимался: сервис, простоявший без
// туннеля, сети не касался, и расхождение снапшота вокруг него — чужое
// целиком.
func (l *Linux) Footprint() []string {
	l.marksMu.Lock()
	defer l.marksMu.Unlock()
